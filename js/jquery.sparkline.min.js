...
var rangex=maxx-minx==0?1:maxx-minx;var rangey=maxy-miny==0?1:maxy-miny;var vl=yvalues.length-1;if(vl<1){this.innerHTML='';
// CloudDot Grounded: Mod to show the single point: Start
            ...
            if(target){
                ...
                if (options.spotRadius && options.spotColor) {
                    var tmpY = canvas_top+Math.round(canvas_height-(canvas_height*((values[0]-miny)/rangey)));
                    target.drawCircle(canvas_left,  tmpY, options.spotRadius, undefined, options.spotColor);
                }
            }
// CloudDot Grounded: Mod to show the single point: End
return;
}
